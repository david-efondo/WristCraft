...
if (isset($_POST['create'])) {
    ...
    if (isset($_FILES['Picture']) && isset($_FILES['Picture']['error']) && $_FILES['Picture']['error'] == 0) {
        ...
        $imageNewName = $target_dir . uniqid() . '.' . $imageExt;
    
        // Move uploaded file
        if (move_uploaded_file($imageTmp, $imageNewName)) {
            $image = $imageNewName;
            error_log("File uploaded successfully: " . $imageNewName);
        } else {
            error_log("Failed to move uploaded file to: " . $imageNewName);
        }
    } else {
        error_log("File upload error: " . ($_FILES['Picture']['error'] ?? 'No file uploaded'));
    }
...
<div class="divContainer">
    ...
    <input type="file" name="Picture" style="padding: 15px; margin-bottom: 15px;">
...
<script>
    ...
    <!-- Display Products -->
    <h1 style="text-align: center; color: white;">Product List</h1>
    <!-- Search Bar -->
    <form method="GET" action="" style="text-align: center; margin-bottom: 20px;">
        <input type="text" name="search" placeholder="Enter product name or brand" style="padding: 10px; width: 200px; border-radius: 5px; border: 2px solid black;">
        <input type="submit" value="Search" style="padding: 10px 20px; border-radius: 4px; border: none; background-color: green; color: white; cursor: pointer;">
    </form>
    ...
    if (isset($_GET['search'])) {
        ...
        $sql = "SELECT * FROM `watches` WHERE Name LIKE '%$searchQuery%' OR Type LIKE '%$searchQuery%' ORDER BY Id ASC";
    } else {
        $sql = "SELECT * FROM `watches` ORDER BY Id ASC";
    }
    ?>
    ...
    <table border="1">
        ...
        <tr>
            ...
            <td><?= $product['Name']; ?></td>
            <td><img src="<?php echo htmlspecialchars($product['Picture']); ?>" width="156px" height="280px"></td>
            <td><?= $product['Type']; ?></td>
